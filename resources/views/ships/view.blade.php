...
@section('content')
    ...
    @permission('trip_add')
    <div style="bottom: 50px; right: 19px;" class="fixed-action-btn " ><a href="{{route('addTrip')}}?ship_id={{$ship->id}}" class="btn-floating btn-large gradient-45deg-light-blue-cyan gradient-shadow tooltipped" data-position="top" data-tooltip="Add new Trip"><i class="material-icons">add</i></a>
    </div>
    @endpermission
@endsection
...
@section('other_java')
    ...
    <script>
        $('.date_range_picker').daterangepicker({
            opens: 'left',
            startDate: moment('{{$start->toDatetimeString()}}', 'YYYY-MM-DD hh:mm:ss'),
            endDate:  moment('{{$end->toDatetimeString()}}', 'YYYY-MM-DD hh:mm:ss'),
            format:'',
            locale: {
                format: 'DD MMMM, YYYY'
            },
    ...
    <script src="{{asset('assets/vendors/amcharts4/themes/animated.js')}}"></script>
